...
export function VersionHistoryPanel({ document: doc }: VersionHistoryPanelProps) {
  ...
  const restoreMutation = useMutation({
    mutationFn: (versionNumber: number) => documentsApi.restoreVersion(doc.id, versionNumber),
    onSuccess: () => {
            ...
            <div className="relative">
              ...
              {versions.map((version, idx) => (
                <div key={version.id} className="relative flex gap-4 pb-6">
                  {/* Timeline dot */}
                  <div
                    className={`mt-1.5 h-6 w-6 shrink-0 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
                      idx === 0
